...
import { fetchPersonalProblems, addPersonalProblem, deletePersonalProblem, checkProblemExists } from './services/personalProblemsService';
...
const App: React.FC = () => {
  ...
  // Navigation State
  const [view, setView] = useState<'dashboard' | 'explorer' | 'myproblems'>('dashboard');
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Load personal problems if authenticated
      if (userId) {
        try {
  ...
  const handleMyProblemsStatusChange = async (id: string, status: Status) => {
    const userId = session?.user?.id;
    const newProgress = await updateProblemProgress(progress, id, { status }, userId);
  ...
  const handleMyProblemsRemarkChange = async (id: string, remarks: string) => {
    const userId = session?.user?.id;
    const newProgress = await updateProblemProgress(progress, id, { remarks }, userId);
    setProgress(newProgress);
    setMyProblemsProgress({
      ...myProblemsProgress,
  ...
  return (
    ...
    >
      {view === 'dashboard' ? (
        <Dashboard progressMap={progress} totalProblemsSeen={problems.length} />
      ) : view === 'myproblems' ? (
        <MyProblems
